...
async function connectMongo() {
  try {
    ...
    // Try to find usage_summary collection — check test db first, then usage_tracking db
...
async function fetchUsageSummaryStats() {
  ...
  for (const doc of docs) {
    ...
    // Handle recorded_at as string (ISO format from your data)
    let date = null;
    if (doc.recorded_at) {
      try {
...
// ─── Debug endpoint to check usage_summary status ───
